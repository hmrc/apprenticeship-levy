...
class LiveAuditor @Inject()(val auditConnector: AuditConnector) extends Auditor {

  override def audit[T](event: ALAEvent)(block: => Future[T])(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[T] = {
    block andThen {
      case Success(_) => auditConnector.sendEvent(event.toDataEvent(200))
      case Failure(t) => {
        val httpStatus = exceptionToMessage(t)
        auditConnector.sendEvent(event.toDataEvent(httpStatus, t))
