...
trait DesController extends ApiController with Logging {
  ...
  protected val desErrorHandler: PartialFunction[Throwable, Result] = {
        ...
        case e: BadRequestException => {
          ...
          BadRequest(Json.toJson[ErrorResponse](DESError(SERVICE_UNAVAILABLE, "BAD_REQUEST", s"Bad request error: ${extractReason(e.getMessage())}")))
        }
        case e: IOException => {
          // $COVERAGE-OFF$
          ...
          // $COVERAGE-ON$
          ServiceUnavailable(Json.toJson[ErrorResponse](DESError(SERVICE_UNAVAILABLE, "IO", s"DES connection error: ${extractReason(e.getMessage())}")))
        }
        case e: GatewayTimeoutException => {
          ...
          // $COVERAGE-ON$
          RequestTimeout(Json.toJson[ErrorResponse](DESError(REQUEST_TIMEOUT, "GATEWAY_TIMEOUT", s"DES not responding error: ${extractReason(e.getMessage())}")))
        }
        case e: NotFoundException => {
          // $COVERAGE-OFF$
          logger.warn(s"Client ${MDC.get("X-Client-ID")} DES error: ${e.getMessage()}, API returning NotFound with code ${NOT_FOUND}")
          // $COVERAGE-ON$
          NotFound(Json.toJson[ErrorResponse](DESError(NOT_FOUND, "NOT_FOUND", s"DES endpoint not found: ${extractReason(e.getMessage())}")))
        }
        case e: UpstreamErrorResponse => {
          ...
          e.statusCode match {
            case PRECONDITION_FAILED =>
              InternalServerError(Json.toJson[ErrorResponse](DESError(420, "BACKEND_FAILURE", s"DES backend error: ${extractReason(e.getMessage())}")))
            case FORBIDDEN => Forbidden(Json.toJson[ErrorResponse](DESError(e.reportAs, "FORBIDDEN", s"DES forbidden error: ${extractReason(e.getMessage())}")))
            case UNAUTHORIZED => Unauthorized(Json.toJson[ErrorResponse](DESError(e.reportAs, "UNAUTHORIZED", s"DES unauthorised error: ${extractReason(e.getMessage())}")))
            case TOO_MANY_REQUESTS =>
              TooManyRequests(Json.toJson[ErrorResponse](DESError(TOO_MANY_REQUESTS, "TOO_MANY_REQUESTS", s"DES too many requests: ${extractReason(e.getMessage())}")))
            case REQUEST_TIMEOUT =>
              RequestTimeout(Json.toJson[ErrorResponse](DESError(REQUEST_TIMEOUT, "TIMEOUT", s"DES not responding error: ${extractReason(e.getMessage())}")))
            case _ =>{
             if(e.statusCode >= 400 && e.statusCode < 500)
               ServiceUnavailable(Json.toJson[ErrorResponse](DESError(e.reportAs, "OTHER", s"DES 4xx error: ${extractReason(e.getMessage())}")))
             else
               ServiceUnavailable(Json.toJson[ErrorResponse](DESError(e.reportAs, "BACKEND_FAILURE", s"DES 5xx error: ${extractReason(e.getMessage())}")))
            }
          }
        }
        case e => {
          ...
          // $COVERAGE-ON$
          InternalServerError(Json.toJson[ErrorResponse](DESError(INTERNAL_SERVER_ERROR, "API", s"API or DES internal server error: ${extractReason(e.getMessage())}")))
        }
    }
}
